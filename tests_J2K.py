...
def main():
    ...
    out_tbl = Path("results-J2K-raw.csv")
    out_sum = Path("summary-J2K-raw.txt")
    decomp = ["CFR", "JDGUI", "Fernflower"]
    ...
    for td in tests:
        row = {"Test": td.name}
        for d in decomp:
            ok = any("J2K" in cls.name for cls in (td / d).glob("*.class")) if (td / d).is_dir() else False
            row[d] = "+" if ok else "-"
            if ok: passed[d] += 1
        rows.append(row)
