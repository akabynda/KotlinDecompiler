...
from dim_reduction.common import numeric_df, safe_corr_df, safe_corr_df_interactive, drop_low_variance

if __name__ == "__main__":
    df = pd.read_csv(Path(input("Path to metrics.csv:")))

    removed: List[str] = []
    num = numeric_df(df, removed)
    num = drop_low_variance(num, removed)
