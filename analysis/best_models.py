...
class MetricProcessor:
    ...
    def compute_distances(self, row: pd.Series, ref_vec: pd.Series, coverage: float) -> pd.Series:
        """
    ...
    def process(self) -> None:
        ...
        distances = df_avg.apply(
            lambda row: self.compute_distances(row, ref_vec, row['case_count'] / total_cases),
            axis=1
        )
