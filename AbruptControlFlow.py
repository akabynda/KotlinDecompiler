...
def count_abrupt_control_flow(node):
    ...
    предполагается, что в AST Kotlin используются типы "break_expression" и "continue_expression".
    """
    count = 0
    ...
    for file in files:
        if file.endswith(".kt"):
            ...
            try:
                ...
                abrupt_count = count_abrupt_control_flow(tree.root_node)
                print(f"{kt_filepath}: Abrupt Control Flow = {abrupt_count}")
                ...
                # Добавляем столбец "Abrupt Control Flow" с рассчитанным значением
                append_column_to_csv(csv_filepath, "Abrupt Control Flow", abrupt_count)
