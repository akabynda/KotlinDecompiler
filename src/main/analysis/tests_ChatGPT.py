...
def main() -> None:
    ...
    tests: List[Path] = sorted([d for d in root.iterdir() if d.is_dir()], key=lambda p: p.name)
    ...
    for test_dir in tests:
        ...
        for decompiler in decompilers:
            ...
            has_chatgpt_class = any(
                "ChatGPT" in cls.name for cls in decompiler_dir.glob("*.class")) if decompiler_dir.is_dir() else False
    ...
    with out_summary.open("w", encoding="utf-8") as f:
        for decompiler in decompilers:
            pct: float = (passed_counts[decompiler] / total_tests * 100) if total_tests else 0
            f.write(f"{decompiler}: {passed_counts[decompiler]}/{total_tests} — {pct:.2f}%\n")
