...
from src.main.utils.gen_len_stats import get_max_new
from src.main.utils.make_prompt import to_bytecode, make_prompt, wrap_as_row
...
class HyperparameterTuner:
    ...
    def __init__(self) -> None:
        ...
        self.rows_for_stats = [
            Row(r["kt_path"], r["kt_source"], to_bytecode(r))
            for r in self.raw_dataset["train"]
        ]
    ...
    def prepare_datasets(self) -> None:
        ...
        self.raw_dataset = DatasetDict(
            {
                split: self.raw_dataset[split].map(
                    lambda ex: make_prompt(wrap_as_row(ex))
                )
        ...
        self.avg_sec_len = int(
            np.percentile([len(r["input_ids"]) for r in self.tokenized_ds["train"]], 95)
        )
                            ...
                            json.dumps(
                                {
                                    ...
                                    "kt_source": extract_kotlin(
                                        rec["text"].split("<|im_end|>\n")[-2]
                                    ),
                                }
                            )
                            + "\n"
    ...
    def compute_metrics(
        self, gen_jsonl: Path, csv_path: Path, metric_list: List[str]
    ) -> None:
        """
    ...
    def compute_row(
        self, args: Tuple[str, str, str, str, List[str]]
    ) -> Optional[List[Any]]:
        """
        ...
        try:
            ...
            return [kt_path, field] + [
                s.get(m, lm.get(m, ent.get(m, 0.0))) for m in metric_list
            ]
        except Exception:
            return None
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        test_subset = sorted(
            list(self.raw_dataset["test"]), key=lambda r: len(r["text"])
        )[:VAL_SUBSET_SIZE]
    ...
    def run_study(self, n_trials: int = 30) -> None:
        ...
        study = optuna.create_study(
            ...
            sampler=optuna.samplers.TPESampler(
                seed=GLOBAL_SEED, multivariate=True, group=True
            ),
