...
class FeatureSelector:
    ...
    def drop_low_variance(df_in: pd.DataFrame, removed: List[str], threshold: float = 0.01) -> pd.DataFrame:
        ...
        """
        scaler = MinMaxScaler()
        scaled_df = pd.DataFrame(scaler.fit_transform(df_in), columns=df_in.columns, index=df_in.index)
    ...
    def drop_high_corr_recursive(df_in: pd.DataFrame, upper: pd.DataFrame, thresh: float,
                                 to_drop: Optional[Set[str]] = None, path: Optional[List[str]] = None,
                                 best_result: Optional[Dict] = None) -> Dict:
        """
        ...
        for row in upper.index:
            for col in upper.columns:
                if upper.at[row, col] > thresh and row not in to_drop and col not in to_drop:
                    ...
                    FeatureSelector.drop_high_corr_recursive(df_in, upper, thresh, new_to_drop_1,
                                                             path + [f"drop {row}"], best_result)

                    new_to_drop_2 = to_drop.copy()
                    new_to_drop_2.add(col)
                    FeatureSelector.drop_high_corr_recursive(df_in, upper, thresh, new_to_drop_2,
                                                             path + [f"drop {col}"], best_result)
