...
def mock_hf_dataset():
    ...
    class FakeDataset:
        ...
        def train_test_split(self, test_size):
            ...
            return {
                "train": FakeDataset(self.items[:n]),
                "test": FakeDataset(self.items[n:]),
            }
...
def test_init_sets_attributes(tmp_path):
    merger = DatasetMerger("ds1", "ds2", tmp_path, test_size=0.3, seed=42)
...
def test_process_full_pipeline(monkeypatch, tmp_path, mock_hf_dataset):
    ...
    split = {
        "train": FakeDataset([{"i": 1}, {"i": 2}, {"i": 3}]),
        "test": FakeDataset([{"i": 4}]),
    }
